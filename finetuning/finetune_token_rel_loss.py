...
def get_token_relation_loss(args, student_hidden_states, teacher_hidden_states, attention_mask=None):
    rel_total = None
    valid_layers = 0
    ...
    for student_idx, teacher_idx in zip(args.student_layer_mapping, args.teacher_layer_mapping):
        ...
        rel_loss = relation_loss_fn(
            student_hidden.float(),
            teacher_hidden.float(),
            attention_mask=attention_mask)
        rel_total = rel_loss if rel_total is None else rel_total + rel_loss
        valid_layers += 1

    if valid_layers == 0:
        return teacher_hidden_states[-1].new_tensor(0.0)

    return rel_total / valid_layers


def get_distil_loss(args, tokenizer, model, teacher_model, model_batch, no_model_batch, logits, student_hidden_states=None):
    with torch.no_grad():
        teacher_model.eval()
        teacher_outputs = teacher_model(
            **model_batch,
            use_cache=False,
            output_hidden_states=student_hidden_states is not None,
            return_dict=True)
    ...
    if student_hidden_states is not None:
        token_rel_loss = get_token_relation_loss(
            args,
            student_hidden_states,
            teacher_outputs.hidden_states,
            attention_mask=model_batch.get("attention_mask"))
        distil_loss = distil_loss + args.w_token_relation_loss * token_rel_loss
...
def finetune(args, tokenizer: AutoTokenizer, model: deepspeed.DeepSpeedEngine, optimizer: AdamW, lr_scheduler, dataset, device, teacher_model=None):
    ...
    def capture_student_hidden_hook(module, inputs, output):
        if module.training:
            student_captured_hidden.append(output[0] if isinstance(output, tuple) else output)

    if teacher_model is not None:
        for layer in model.base_model.model.model.layers:
            hook_handles.append(layer.register_forward_hook(capture_student_hidden_hook))
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data, _, _) in enumerate(train_dataloader):
            ...
            if teacher_model is not None:
                if len(student_captured_hidden) <= 1:
                    raise RuntimeError("Student hidden hook did not capture any hidden state.")
